    ...
    return {
        ...
        return new Promise((resolve, reject) => {
          if (imgExt.includes(file.ext)) {
            ...
            sharp(file.buffer)
              ...
              .toFile(
                path.join(uploadDir, `/uploads/${file.hash}.webp`),
                (err, info) => {
                  file.caption = `/uploads/${file.hash}.webp`;
                  resolve();
                }
              );
          } else {
      ...
      delete(file) {
        return new Promise((resolve, reject) => {
          ...
          const filehWebp = path.join(uploadDir, `uploads/${file.hash}.webp`);

          if (!fs.existsSync(filePath)) {
            return resolve("File doesn't exist");
          }
          if (fs.existsSync(filehWebp)) {
            fs.unlink(filehWebp, (err) => {
              if (err) {
                return reject(err);
              }
            });
